...
var console = {
  ...
  render: function(){
    var fs = require('fs');
    var head = fs.readFileSync('node_modules/mocha-html-reporter/docs/head.html');
    var tail = fs.readFileSync('node_modules/mocha-html-reporter/docs/tail.html');
    if(!fs.existsSync("mocha_html")){
     fs.mkdirSync("mocha_html", 0766, function(err){
       if(err){ 
         throw err;
       }
     });   
    }
    fs.createReadStream('node_modules/mocha-html-reporter/css/mocha.css').pipe(fs.createWriteStream('mocha_html/mocha.css'));
    fs.writeFileSync('mocha_html/result.html', head + this.content + tail);
  }
}
